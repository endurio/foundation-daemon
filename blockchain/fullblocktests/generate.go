...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := ndrutil.Hash160(redeemScript)
...
// subsidy.
func additionalCoinbasePoW(amount ndrutil.Amount) func(*wire.MsgBlock) {
...
func additionalCoinbaseDev(amount ndrutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		// Increase the first proof-of-work coinbase subsidy by the
		// provided amount.
...
func additionalSpendFee(fee ndrutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	g.NextBlock("bbadtaxscript", outs[5], ticketOuts[5],
		func(b *wire.MsgBlock) {
			...
			p2shTaxAddr := addrs[0].(*ndrutil.AddressScriptHash)
			p2pkhTaxAddr, err := ndrutil.NewAddressPubKeyHash(
	...
	g.NextBlock("bv15", outs[9], ticketOuts[9], func(b *wire.MsgBlock) {
		ticketFee := ndrutil.Amount(2)
		ticketPrice := ndrutil.Amount(g.CalcNextReqStakeDifficulty(g.Tip()))
		ticketPrice--
	...
	g.NextBlock("bcb9", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = ndrutil.MaxAmount + 1
	})
	rejected(blockchain.ErrBadTxOutValue)
	...
	g.NextBlock("bcb10", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = ndrutil.MaxAmount
