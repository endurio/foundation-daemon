...
func main() {
	...
	method := args[0]
	usageFlags, err := ndrjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := ndrjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// ndrjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(ndrjson.Error); ok {
		...
		// The error is not a ndrjson.Error and this really should not
	...
	marshalledJSON, err := ndrjson.MarshalCmd("1.0", 1, cmd)
	if err != nil {
