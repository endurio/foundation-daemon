...
type cpuminerConfig struct {
	...
	ProcessBlock func(*ndrutil.Block, blockchain.BehaviorFlags) (bool, error)
...
func (m *CPUMiner) submitBlock(block *ndrutil.Block) bool {
	...
	minrLog.Infof("Block submitted via CPU miner accepted (hash %s, "+
		...
		ndrutil.Amount(coinbaseTx.Value))
	return true
}
	...
	for extraNonce := uint64(0); extraNonce < maxExtraNonce; extraNonce++ {
		...
		for i := uint32(0); i <= maxNonce; i++ {
			...
			case <-ticker.C:
				...
				m.g.UpdateBlockTime(&msgBlock.Header)
...
out:
	for {
		...
		if m.solveBlock(template.Block, curHeight+1, ticker, quit) {
			block := ndrutil.NewBlock(template.Block)
			m.submitBlock(block)
		}
	}
