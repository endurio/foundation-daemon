...
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  ...
  [dcrwallet](https://github.com/endurio/dcrwallet).
...
such as [dcrwallet](https://github.com/endurio/dcrwallet) for inter-process
...
ndrd is `wss://your_ip_or_domain:9109/ws`.
...
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the ndrd
  ...
  in the ndrd home directory (which is typically `%LOCALAPPDATA%\Dcrd` on
...
ndrd comes with a separate utility named `dcrctl` which can be used to issue
...
API compatible with the original ndrd client.
...
|25|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><br />NOTE: Since ndrd does not have the wallet integrated to provide payment addresses, ndrd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
...
|28|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><br />NOTE: ndrd does not yet implement the `allowhighfees` parameter, so it has no effect|
|29|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since ndrd does not have the wallet integrated to provide payment addresses, ndrd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|30|[stop](#stop)|N|Shutdown ndrd.|
|31|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|32|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since ndrd does not have a wallet integrated, ndrd will only return whether the address is valid or not.|
...
|36|[getcurrentnet](#getcurrentnet)|Y|Get Decred network ndrd is running on.|
...
|Notes|NOTE: Since ndrd does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in dcrwallet for a version which includes that information.|
...
|Example Return|`[{"addr": "178.172.xxx.xxx:9108", "services": "00000001", "lastrecv": 1388183523, "lastsend": 1388185470, "bytessent": 287592965, "bytesrecv": 780340, "conntime": 1388182973, "pingtime": 405551, "pingwait": 183023, "version": 70001, "subver": "/ndrd:0.4.0/", "inbound": false, "startingheight": 276921, "currentheight": 276955, "syncnode": true }, ...]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|Since ndrd does not have the wallet integrated to provide payment addresses, ndrd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.
...
|Notes|Since ndrd does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.|
...
|Notes|ndrd does not yet implement the `allowhighfees` parameter, so it has no effect.|
...
|Returns|`(json object)`<br />`isvalid`: `(bool)` whether or not the address is valid.<br />`address`: `(string)` the Decred address validated.<br /><br />`{"isvalid": true or false,"address": "endurioaddress"}`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database. The actual checks performed by the `checklevel` parameter is implementation specific. <br /><br />For ndrd this is: <br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|Dcrd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for ndrd.|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `NDRD`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|`string`|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR NDR CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get Decred network ndrd is running on.|
|Returns|`numeric`|
...
|Parameters|1. `Addresses`: `(JSON array, required)`<br />`endurioaddress`: `(string)` the Decred address.<br /><br />`["endurioaddress", ...]`|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this ndrd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSMethodOverview)<br />
...
[rpcclient](https://github.com/endurio/ndrd/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/endurio/ndrd/tree/master/rpcclient) package to connect with
