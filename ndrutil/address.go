...
func encodePKAddress(serializedPK []byte, netID [2]byte, algo ndrec.SignatureType) string {
	pubKeyBytes := []byte{0x00}

	switch algo {
	case ndrec.STEcdsaSecp256k1:
		pubKeyBytes[0] = byte(ndrec.STEcdsaSecp256k1)
	case ndrec.STEd25519:
		pubKeyBytes[0] = byte(ndrec.STEd25519)
	case ndrec.STSchnorrSecp256k1:
		pubKeyBytes[0] = byte(ndrec.STSchnorrSecp256k1)
	}

	// Pubkeys are encoded as [0] = type/ybit, [1:33] = serialized pubkey
	compressed := serializedPK
	if algo == ndrec.STEcdsaSecp256k1 || algo == ndrec.STSchnorrSecp256k1 {
...
type Address interface {
	...
	DSA(*chaincfg.Params) ndrec.SignatureType
...
func NewAddressPubKey(decoded []byte, net *chaincfg.Params) (Address, error) {
	if len(decoded) == 33 {
		...
		switch ndrec.SignatureType(suite) {
		case ndrec.STEcdsaSecp256k1:
...
func DecodeAddress(addr string) (Address, error) {
	...
	case net.PubKeyHashAddrID:
		return NewAddressPubKeyHash(decoded, net, ndrec.STEcdsaSecp256k1)

	case net.PKHEdwardsAddrID:
		return NewAddressPubKeyHash(decoded, net, ndrec.STEd25519)

	case net.PKHSchnorrAddrID:
		return NewAddressPubKeyHash(decoded, net, ndrec.STSchnorrSecp256k1)
...
// be 20 bytes.
func NewAddressPubKeyHash(pkHash []byte, net *chaincfg.Params, algo ndrec.SignatureType) (*AddressPubKeyHash, error) {
	var addrID [2]byte
	switch algo {
	case ndrec.STEcdsaSecp256k1:
...
// hash.
func (a *AddressPubKeyHash) DSA(net *chaincfg.Params) ndrec.SignatureType {
	switch a.netID {
	case net.PubKeyHashAddrID:
		return ndrec.STEcdsaSecp256k1
	case net.PKHEdwardsAddrID:
		return ndrec.STEd25519
	case net.PKHSchnorrAddrID:
		return ndrec.STSchnorrSecp256k1
	}
	return -1
}
...
func (a *AddressScriptHash) DSA(net *chaincfg.Params) ndrec.SignatureType {
	return -1
}
...
func NewAddressSecpPubKey(serializedPubKey []byte,
	...
	// from ndrec, but do it here to avoid API churn.  We already know the
...
// address.
func (a *AddressSecpPubKey) DSA(net *chaincfg.Params) ndrec.SignatureType {
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressEdwardsPubKey) DSA(net *chaincfg.Params) ndrec.SignatureType {
	return ndrec.STEd25519
}
...
// DSA returns the underlying digital signature algorithm for the
// address.
func (a *AddressSecSchnorrPubKey) DSA(net *chaincfg.Params) ndrec.SignatureType {
	return ndrec.STSchnorrSecp256k1
}
