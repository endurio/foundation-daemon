...
func NewWIF(privKey chainec.PrivateKey, net *chaincfg.Params, ecType ndrec.SignatureType) (*WIF,
	error) {
	if net == nil {
...
func DecodeWIF(wif string) (*WIF, error) {
	...
	var ecType ndrec.SignatureType
	switch ndrec.SignatureType(decoded[2]) {
	...
	case ndrec.STEd25519:
		...
		ecType = ndrec.STEd25519
	case ndrec.STSchnorrSecp256k1:
		privKeyBytes := decoded[3 : 3+secp256k1.PrivKeyBytesLen]
		privKey, _ = secp256k1.PrivKeyFromScalar(privKeyBytes)
		ecType = ndrec.STSchnorrSecp256k1
	}
...
func (w *WIF) DSA() ndrec.SignatureType {
	return w.ecType
}
