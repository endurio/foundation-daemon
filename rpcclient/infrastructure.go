...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *ndrjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *ndrjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *ndrjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled ndrjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := ndrjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := ndrjson.MarshalCmd("1.0", id, cmd)
	if err != nil {
		return newFutureError(err)
	}
