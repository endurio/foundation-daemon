...
func (r FutureGetRawTransactionResult) Receive() (*ndrutil.Tx, error) {
	...
	return ndrutil.NewTx(&msgTx), nil
}
...
func (c *Client) GetRawTransactionAsync(txHash *chainhash.Hash) FutureGetRawTransactionResult {
	...
	cmd := ndrjson.NewGetRawTransactionCmd(hash, ndrjson.Int(0))
...
// transaction.
func (c *Client) GetRawTransaction(txHash *chainhash.Hash) (*ndrutil.Tx, error) {
...
func (r FutureGetRawTransactionVerboseResult) Receive() (*ndrjson.TxRawResult, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
	...
	var rawTxResult ndrjson.TxRawResult
...
func (c *Client) GetRawTransactionVerboseAsync(txHash *chainhash.Hash) FutureGetRawTransactionVerboseResult {
	...
	cmd := ndrjson.NewGetRawTransactionCmd(hash, ndrjson.Int(1))
	return c.sendCmd(cmd)
}
...
func (c *Client) GetRawTransactionVerbose(txHash *chainhash.Hash) (*ndrjson.TxRawResult, error) {
...
func (r FutureDecodeRawTransactionResult) Receive() (*ndrjson.TxRawResult, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
...
func (c *Client) DecodeRawTransactionAsync(serializedTx []byte) FutureDecodeRawTransactionResult {
	...
	cmd := ndrjson.NewDecodeRawTransactionCmd(txHex)
	return c.sendCmd(cmd)
}

// DecodeRawTransaction returns information about a transaction given its
// serialized bytes.
func (c *Client) DecodeRawTransaction(serializedTx []byte) (*ndrjson.TxRawResult, error) {
...
func (c *Client) CreateRawTransactionAsync(inputs []ndrjson.TransactionInput,
	amounts map[ndrutil.Address]ndrutil.Amount, lockTime *int64, expiry *int64) FutureCreateRawTransactionResult {

	convertedAmts := make(map[string]float64, len(amounts))
	for addr, amount := range amounts {
		convertedAmts[addr.String()] = amount.ToCoin()
	}
	cmd := ndrjson.NewCreateRawTransactionCmd(inputs, convertedAmts, lockTime, expiry)
	return c.sendCmd(cmd)
}

// CreateRawTransaction returns a new transaction spending the provided inputs
// and sending to the provided addresses.
func (c *Client) CreateRawTransaction(inputs []ndrjson.TransactionInput,
	amounts map[ndrutil.Address]ndrutil.Amount, lockTime *int64, expiry *int64) (*wire.MsgTx, error) {
...
func (c *Client) SendRawTransactionAsync(tx *wire.MsgTx, allowHighFees bool) FutureSendRawTransactionResult {
	...
	cmd := ndrjson.NewSendRawTransactionCmd(txHex, &allowHighFees)
	return c.sendCmd(cmd)
}
...
func (r FutureSignRawTransactionResult) Receive() (*wire.MsgTx, bool, error) {
	...
	var signRawTxResult ndrjson.SignRawTransactionResult
...
func (c *Client) SignRawTransactionAsync(tx *wire.MsgTx) FutureSignRawTransactionResult {
	...
	cmd := ndrjson.NewSignRawTransactionCmd(txHex, nil, nil, nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) SignRawTransaction2Async(tx *wire.MsgTx, inputs []ndrjson.RawTxInput) FutureSignRawTransactionResult {
	txHex := ""
	if tx != nil {
		// Serialize the transaction and convert to hex string.
		buf := bytes.NewBuffer(make([]byte, 0, tx.SerializeSize()))
	...
	cmd := ndrjson.NewSignRawTransactionCmd(txHex, &inputs, nil, nil)
	return c.sendCmd(cmd)
}
...
// transactions.
func (c *Client) SignRawTransaction2(tx *wire.MsgTx, inputs []ndrjson.RawTxInput) (*wire.MsgTx, bool, error) {
...
func (c *Client) SignRawTransaction3Async(tx *wire.MsgTx,
	...
	cmd := ndrjson.NewSignRawTransactionCmd(txHex, &inputs, &privKeysWIF,
		nil)
	return c.sendCmd(cmd)
}
...
func (c *Client) SignRawTransaction4Async(tx *wire.MsgTx,
	inputs []ndrjson.RawTxInput, privKeysWIF []string,
	...
	cmd := ndrjson.NewSignRawTransactionCmd(txHex, &inputs, &privKeysWIF,
		ndrjson.String(string(hashType)))
	return c.sendCmd(cmd)
}
...
func (c *Client) SignRawSSGenTxAsync(tx *wire.MsgTx) FutureSignRawTransactionResult {
	...
	cmd := ndrjson.NewSignRawTransactionCmd(txHex, &[]ndrjson.RawTxInput{},
		nil, ndrjson.String("ssgen"))
	return c.sendCmd(cmd)
}
...
func (c *Client) SearchRawTransactionsAsync(address ndrutil.Address, skip,
	count int, reverse bool,
	filterAddrs []string) FutureSearchRawTransactionsResult {

	addr := address.EncodeAddress()
	verbose := ndrjson.Int(0)
	prevOut := ndrjson.Int(0)
	cmd := ndrjson.NewSearchRawTransactionsCmd(addr, verbose, &skip, &count,
...
func (c *Client) SearchRawTransactions(address ndrutil.Address, skip, count int,
...
// Receive waits for the response promised by the future and returns the
// found raw transactions.
func (r FutureSearchRawTransactionsVerboseResult) Receive() ([]*ndrjson.SearchRawTransactionsResult, error) {
	res, err := receiveFuture(r)
	if err != nil {
		return nil, err
	}
	...
	var result []*ndrjson.SearchRawTransactionsResult
...
func (c *Client) SearchRawTransactionsVerboseAsync(address ndrutil.Address, skip,
	count int, includePrevOut bool, reverse bool,
	filterAddrs *[]string) FutureSearchRawTransactionsVerboseResult {

	addr := address.EncodeAddress()
	verbose := ndrjson.Int(1)
	prevOut := ndrjson.Int(0)
	if includePrevOut {
		prevOut = ndrjson.Int(1)
	}
	cmd := ndrjson.NewSearchRawTransactionsCmd(addr, verbose, &skip, &count,
		prevOut, &reverse, filterAddrs)
	return c.sendCmd(cmd)
...
func (c *Client) SearchRawTransactionsVerbose(address ndrutil.Address, skip,
	count int, includePrevOut bool, reverse bool,
	filterAddrs []string) ([]*ndrjson.SearchRawTransactionsResult, error) {
