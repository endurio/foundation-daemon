...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt ndrutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(ndrutil.Amount(5 * ndrutil.AtomsPerCoin))
	...
	txid = genSpend(ndrutil.Amount(5000 * ndrutil.AtomsPerCoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testJoinMempools(r *Harness, t *testing.T) {
	...
	pooledHashes, err := r.Node.GetRawMempool(ndrjson.GRMAll)
	if err != nil {
	...
	go func() {
		for {
			poolHashes, err := r.Node.GetRawMempool(ndrjson.GRMAll)
			if err != nil {
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := ndrutil.Amount(5 * 300 * ndrutil.AtomsPerCoin)
	...
	// chain.
	expectedBalance = ndrutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := ndrutil.Amount(50 * ndrutil.AtomsPerCoin)
...
func TestHarness(t *testing.T) {
	...
	expectedBalance := ndrutil.Amount(numMatureOutputs * 300 * ndrutil.AtomsPerCoin)
