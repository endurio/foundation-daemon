...
// FileContents is a string containing the commented example config for ndrd.
...
; The default is ~/.ndrd/data on POSIX OSes, $LOCALAPPDATA/Dcrd/data on Windows,
; ~/Library/Application Support/Dcrd/data on macOS, and $homed/ndrd/data on
; Plan9.  Environment variables are expanded so they may be used.  NOTE: Windows
; environment variables are typically %VARIABLE%, but they must be accessed with
; $VARIABLE here.
; datadir=~/.ndrd/data                            ; Unix
...
; line.  ndrd will not contact 3rd-party sites to obtain external ip addresses.
...
; with, but the behavior is slightly different.  By default, ndrd will query DNS
...
; Disable DNS seeding for peers.  By default, when ndrd starts, it will use
...
; which is used to control and query information from a running ndrd process.
;
...
; log level for individual subsystems.  Use ndrd --debuglevel=show to list
