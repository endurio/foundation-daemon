...
// ndrdService houses the main service handler which handles all service
// updates and launching ndrdMain.
type ndrdService struct{}
...
// long-running ndrdMain (which is the real meat of ndrd), handles service
// change requests, and notifies the service control manager of changes.
func (s *ndrdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start ndrdMain in a separate goroutine so the service can start
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &ndrdService{})
	if err != nil {
