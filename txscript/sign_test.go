...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr ndrutil.Address) (chainec.PrivateKey,
			bool, error) {
...
func mkGetScript(scripts map[string][]byte) ScriptDB {
	if scripts == nil {
		return ScriptClosure(func(addr ndrutil.Address) (
			[]byte, error) {
			return nil, errors.New("nope 3")
		})
	}
	return ScriptClosure(func(addr ndrutil.Address) ([]byte,
		error) {
...
func signAndCheck(msg string, tx *wire.MsgTx, idx int, pkScript []byte,
	...
	previousScript []byte, suite ndrec.SignatureType) error {
...
func TestSignTxOutput(t *testing.T) {
	...
	signatureSuites := []ndrec.SignatureType{
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := ndrutil.NewAddressPubKeyHash(
					ndrutil.Hash160(pkBytes), testingParams,
					suite)
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			if err := signAndCheck(msg, tx, i, pkScript, hashType,
				...
				ndrec.STEcdsaSecp256k1); err != nil {
				t.Error(err)
				break
			}
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				// for more details
				address, err := ndrutil.NewAddressSecpPubKeyCompressed(pk,
					testingParams)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
		...
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				case ndrec.STEd25519:
					...
					key, pk = chainec.Edwards.PrivKeyFromBytes(keyDB)
					pkBytes = pk.SerializeUncompressed()
					address, err = ndrutil.NewAddressEdwardsPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
				...
				case ndrec.STSchnorrSecp256k1:
					...
					key, pk = chainec.SecSchnorr.PrivKeyFromBytes(keyDB)
					pkBytes = pk.Serialize()
					address, err = ndrutil.NewAddressSecSchnorrPubKey(pkBytes,
						testingParams)
					if err != nil {
						t.Errorf("failed to make address for %s: %v",
							msg, err)
	...
	for _, hashType := range hashTypes {
		for _, suite := range signatureSuites {
			for i := range tx.TxIn {
				...
				address, err := ndrutil.NewAddressPubKeyHash(
					ndrutil.Hash160(pkBytes), testingParams, suite)
				if err != nil {
					t.Errorf("failed to make address for %s: %v",
						msg, err)
					break
				...
				scriptAddr, err := ndrutil.NewAddressScriptHash(
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			address1, err := ndrutil.NewAddressSecpPubKeyCompressed(pk1,
				testingParams)
			if err != nil {
				t.Errorf("failed to make address for %s: %v",
					msg, err)
			...
			address2, err := ndrutil.NewAddressSecpPubKeyCompressed(pk2,
				testingParams)
			if err != nil {
			...
			pkScript, err := MultiSigScript(
				[]*ndrutil.AddressSecpPubKey{address1, address2},
				2)
			if err != nil {
				t.Errorf("failed to make pkscript "+
					"for %s: %v", msg, err)
			...
			if err := signAndCheck(msg, tx, i, scriptPkScript,
				...
				mkGetKey(map[string]addressToKey{
					...
					scriptAddr.EncodeAddress(): pkScript,
				}), nil, ndrec.STEcdsaSecp256k1); err != nil {
				t.Error(err)
				break
			}
...
var (
	...
	thisAddressUnc, _ = ndrutil.NewAddressPubKeyHash(
		ndrutil.Hash160(thisPubKey.SerializeUncompressed()),
		testingParams, ndrec.STEcdsaSecp256k1)
	uncompressedPkScript, _ = PayToAddrScript(thisAddressUnc)
	thisAddressCom, _       = ndrutil.NewAddressPubKeyHash(
		ndrutil.Hash160(thisPubKey.SerializeCompressed()),
		testingParams, ndrec.STEcdsaSecp256k1)
